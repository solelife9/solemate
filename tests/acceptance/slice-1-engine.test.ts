...
describe('Scenario 3: 자동 일시정지/재개가 실제로 작동한다', () => {
  test('0.6 m/s 미만 6초 지속 → 자동 일시정지', () => {
    let s = initAutoPauseState();
    let r = decideAutoPause(s, 0.3, 3); // 3초 정지
    expect(r.paused).toBe(false);
    r = decideAutoPause(r.state, 0.3, 3.5); // 누적 6.5초
    expect(r.paused).toBe(true);
    expect(r.justPaused).toBe(true);
  });

  test('1.0 m/s 초과 2초 지속 → 자동 재개', () => {
    let s = initAutoPauseState();
    let r = decideAutoPause(s, 0.2, 7); // paused
    expect(r.paused).toBe(true);
    r = decideAutoPause(r.state, 1.5, 2.5); // 다시 달림
    expect(r.paused).toBe(false);
