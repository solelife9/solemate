...
> **그리고 2026-08-07 에 Wear OS 워치 앱도 범위에 들어왔다** — iOS 는 애플워치 단독 러닝이
> 되는데 Android 는 워치 앱이 없어서, '완벽하게'를 플랫폼 동등까지로 본다는 결정이다.
> 규모가 **90시간(전체의 40%)** 이고 **Wear OS 실기기가 없으면 착수 불가**다 —
> 분해와 근거는 `docs/audit/FINAL-readiness.md §5 D`.
