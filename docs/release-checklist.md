...
### 남은 것
- 🔴 👤 **Firebase Storage 규칙 배포 — `firebase deploy --only storage`** (2026-08-07 신설).
  메달·기록증 사진 백업이 Storage 를 쓰기 시작했다(`lib/photoCloud`). **규칙(`storage.rules`)을
  올리지 않으면 기본 규칙이 업로드를 전부 거부한다** — 그런데 코드는 실패를 삼키도록 설계돼
  있어서(사진 백업이 메달 등록을 막으면 안 되므로) **아무 에러 없이 백업만 조용히 안 된다.**
  겉보기엔 멀쩡하고 재설치해야 비로소 드러나는 종류의 사고다.
  · 콘솔에서 **Storage 를 한 번도 활성화하지 않았다면 먼저 활성화**해야 한다(버킷 생성).
  · 확인법: 로그인 → 메달에 사진 등록 → 콘솔 Storage 에 `users/{uid}/medals/…jpg` 가 보이면 성공.
- 🔴 👤 **랭킹 서버 배포 — `firebase deploy --only functions,firestore:rules`** (2026-08-07 신설).
  리더보드 점수를 서버가 다시 계산하도록 바꿨다(`functions/ranking.js`). **둘을 같이 올려야 한다** —
  규칙만 올리면 앱이 못 쓰는데 서버도 없어서 **랭킹이 통째로 멎고**, 함수만 올리면 옛 규칙이
  클라이언트 쓰기를 계속 허용해 위조 차단이 무의미하다.
  · 확인법: 로그인 → 러닝 저장 → 동기 후 콘솔에서 `leaderboards/{이번달}/entries/{uid}` 의
    `distance` 가 실제 그달 거리와 같은지. 함수 로그에 `ranking/publish` 오류가 없는지.
