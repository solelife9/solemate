...
| A-4 | **Android 서명 지문 미등록 → 로그인 실패** | Android | `BLOCKER` |
| A-5 | ~~JS 번들이 3주째 낡음~~ **해결**(cb0c07e) | Android | ~~`BLOCKER`~~ 해결 |
...
### A-4. `BLOCKER` Android 서명 지문이 개발용만 등록돼 있다 — 출시본에서 로그인이 전부 실패한다

에뮬레이터에 **릴리스 APK** 를 올려 로그인을 시도하다 발견했다.

```
google-services.json 에 등록된 SHA-1 : 5E:8F:16:06:…:F6:25  ← debug.keystore
릴리스 APK 의 실제 SHA-1            : 39:D4:16:D7:…:67:CE  ← 미등록 ★
```

**왜 iOS 는 멀쩡한가:** iOS 는 번들 ID 만으로 앱을 식별한다 — 서명 지문 개념이 없다.
Android 는 **패키지명 + 서명 인증서**가 한 쌍으로 신원이라, 키가 다르면 구글·카카오
입장에서 **다른 앱**이다. 그래서 이 문제는 Android 에만 존재한다.

| 로그인 | 지문 필요 | 현재 |
|---|---|---|
| Google | Firebase 에 SHA-1 | ❌ 개발용만 |
| 카카오 | 카카오에 키 해시 | ❌ 개발용만(추정 — 콘솔 확인 필요) |
| 네이버 | 불필요(client id/secret) | ✅ 동작 예상 |

**등록할 값**

| 용도 | 값 |
|---|---|
| Firebase SHA-1 (출시 키) | `39:D4:16:D7:52:BE:F6:34:17:D4:A2:2F:20:F4:69:C3:EE:12:67:CE` |
| 카카오 키 해시 (출시 키) | `OdQW11K+9jQX1KIvIPRpw+4SZ84=` |
| 카카오 키 해시 (개발 키, 참고) | `Xo8WBi6jzSxKDVR4drqm84yr9iU=` |

Firebase 는 지문 추가 후 **`google-services.json` 을 다시 받아** `android/app/` 에 덮어써야 한다.

> #### ⚠️ 지문이 하나 더 있다 — 가장 흔히 밟는 지뢰
> Play 에 업로드하면 **구글이 자기 키로 앱을 재서명한다**(Play App Signing). 실제 사용자
> 기기에 설치되는 앱의 지문은 위 둘 중 어느 것도 아닌 **제3의 값**이다.
> 첫 업로드 후 **Play Console → 설정 → 앱 서명**에서 그 SHA-1 을 확인해 Firebase·카카오에
> **똑같이 등록**해야 한다. 놓치면 **스토어에 올린 앱에서만 로그인이 전부 실패**하고,
> 내부 테스트에서는 재현되지 않아 원인을 찾기 어렵다.

### A-5. ~~`BLOCKER`~~ **해결됨(2026-07-30, `cb0c07e`)** — JS 번들이 3주째 낡아 있었다

에뮬레이터에서 카카오 버튼 로고가 공식 말풍선이 아니라 **옛 글자 "K"** 로 나온 것이 단서였다.
`android/app/src/main/assets/index.android.bundle` 이 **2026-07-08 자**였고, 그 사이 3주치
화면·문구 변경이 Android 에만 통째로 빠져 있었다.

원인은 구조다: `build.gradle:206` 이 in-Gradle 번들링을 Gradle 9 호환 문제로 **일부러 꺼놨고**,
대신 `npm run bundle:android` 로 미리 구운 번들을 패키징한다. 그 명령을 깜빡하면 옛 번들이
그대로 포장되는데 — **빌드는 성공하고 앱도 정상으로 켜져서 아무도 모른다.**

조치: 번들 재생성(로고·글자 잘림 동시 해소) + **신선도 검사**를 추가해 소스가 번들보다
새로우면 release 빌드를 세운다. 서명 가드와 같은 원칙 — 사람의 기억에 기댄 절차는
안전장치가 아니다.
...
3. **A-4 서명 지문 등록**(Firebase·카카오) — 안 하면 Android 로그인이 전부 죽는다. Play 재서명 지문은 첫 업로드 후 추가.
4. **B-3 심박(Health Connect) 범위 결정** — 동시 출시의 실질적 최대 변수. 넣으면 일정↑, 빼면 Android는 반쪽.
5. ~~B-1 익명 로그인 결정~~ **완료** — 소셜 전용 확정(계정 필수).
6. **B-4 앱 ID 결정** — 되돌릴 수 없으므로 지금.
7. A-2·A-3·C는 콘솔/실빌드 작업.
