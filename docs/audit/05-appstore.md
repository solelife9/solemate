...
## 9.5 종결 (2026-08-03 — N-4 + 소셜 개봉)

**N-4 종결 — 사진은 안전하다.** 민우님 확인 결과 **무료 스톡**(Unsplash/Pexels 계열)이고,
파일을 직접 열어 보니 **얼굴이 없는 다리 모션블러**다. 식별 가능한 인물이 없어 초상권 이슈가
없고, 신발도 흐려져 로고가 읽히지 않아 상표 이슈도 없다. 무료 스톡 라이선스(상업적 사용 허용)
범위 안이다 — **교체 불필요.**

**③ 처리방침 배포 완료 → 소셜 개봉.** 감사가 "남은 건 이것 하나"라고 적었던 항목이 끝났다.
순서는 **고지 먼저, 개봉 나중**으로 지켰다(뒤집으면 B-1 사고가 그대로 재현된다):

1. `docs/privacy.html` 제4조의2 신설 — 공개되는 항목을 표로, **공개되지 않는 항목**
   (경로·신체정보·메모·사진)을 못 박아, 공개 중단/탈퇴 시 삭제, 대상은 '로그인한 다른
   이용자'(인터넷 전체 아님).
2. **공개 저장소 `solelife9/keego-legal` 푸시** → `privacy.html` **200 + 조항 반영 확인**
   (시행일 2026-08-03). *이 저장소만 고치는 건 고지가 아니다 — 공개 URL 이 정본이다.*
3. `docs/store-privacy-labels.md` §2-0 신설. **Play「데이터 공유」는 "없음" 유지** —
   Play 의 '공유'는 다른 회사로의 전송이고, 같은 서비스 이용자 간 노출은 해당하지 않는다.
4. `LEADERBOARD_PUBLISH_ENABLED` + `SOCIAL_PROFILE_PUBLISH_ENABLED` **둘 다 true**.
   `docs/store-listing.md` 에 "월간 랭킹" 복원(이제 실제로 동작하므로 광고해도 된다).
5. `firestore.rules` 배포 완료.

**가드를 "꺼져 있음"에서 "일관성"으로 다시 썼다.** `flag === false` 고정은 켜는 순간 의미를
잃는다(실제로 하루 만에 켰다). 새 불변식은 **발행 ON ⟺ 동의화면·뷰어·처리방침 고지·스토어
신고가 갖춰짐**이고, 어느 방향으로 어긋나도 빨개진다 — 고지를 지우고 발행만 켜둬도,
발행을 껐는데 스토어가 계속 광고해도.

**함께 정정:** 처리방침이 검색어·등록요청을 "계정 식별자와 연결됩니다"라고 적고 있었는데
`6852e02` 이후 사실이 아니다. **공개 문서가 앱보다 더 많이 수집한다고 말하던 상태**라 같은
배포에서 고쳤다.

> ⚠️ **남은 리스크 하나(리젝 사유 아님).** 랭킹 점수는 여전히 **클라이언트가 계산**한다.
> 규칙이 막는 건 '남의 엔트리 변조'와 '형태'뿐이라 자기 점수 부풀리기는 막지 못한다.
> 서버 재계산(Cloud Function)이 붙기 전까지 **순위를 상금·보상과 연결하지 않는다** —
> 자랑 용도까지가 안전선이다. `lib/featureFlags.ts` 주석에 같은 경고를 남겼다.

## 9.6 남은 것 — **감사 지적은 전부 종결**

~~N-4 사진 라이선스~~ · ~~설정 토글 무력화~~ · ~~③ 처리방침 배포~~ → 전부 §9.5 에서 해소.
설정의 '프로필 공개' 토글은 플래그를 켜면서 **실제로 동작하게 됐다**(유령 어포던스 해소).

**이 감사에서 나온 항목은 남은 게 없다.** 아래는 감사 범위 밖이지만 소셜 개봉으로 새로
생긴 후속 과제라 이어서 적어 둔다.

| 항목 | 성격 | 왜 지금은 안 하나 |
|---|---|---|
| 랭킹 점수 **서버 재계산**(Cloud Function) | 남용 방지 | 클라이언트 계산이라 자기 점수 부풀리기가 가능하다. 리젝 사유는 아니고, 순위를 보상과 연결하지 않는 한 피해가 없다. 연결하려면 **이게 선행조건**이다 |
| 실기기에서 소셜 한 바퀴 | 검증 | 동의 → 발행 → 남의 프로필 열기 → 공개 중단 시 내려가는지. 시뮬로는 Firestore 왕복을 못 본다 |
| 리더보드 읽기 비용(AUDIT 2 I-6) | 비용 | 진입 1회에 약 203 읽기. 규칙이 막고 있어 **실행되지 않는 코드**였는데, 이제 열렸으므로 실제 비용이 발생한다 — 사용자가 늘면 재검토 |
