...
// ── P1 #8) 기타(커스텀 브랜드) 직접 입력 + 커스텀 모델 저장 ───────────────────────
test('기타 브랜드를 골라 직접 입력하면 그 브랜드 + 커스텀 모델로 저장된다(P1 #8)', async () => {
  const onSave = jest.fn();
  const root = await mountScreen(onSave);

  // '기타' 브랜드 칩 선택 → 브랜드 직접 입력칸 등장
  await tap(pressBy(root, '기타'));
  const brandInput = root.findAll((n: any) => n && n.props && n.props.testID === 'add-shoe-custom-brand')[0];
  expect(brandInput).toBeTruthy();
  await act(async () => { brandInput.props.onChangeText('Salomon'); });
  await flush();

  // DB에 없는 모델 → '직접 추가'로 커밋
  await typeModel(root, 'Speedcross 6');
...
test('모델 모달을 열면 브랜드(Nike) 전체 모델이 알파벳순으로 뜬다', async () => {
  ...
  expect(shown.length).toBeGreaterThan(15); // 통합 카탈로그: Nike 다수 모델
  const sorted = [...shown].sort((a, b) => a.localeCompare(b));
  expect(shown).toEqual(sorted); // 알파벳순
...
test('모델명을 검색하면 전체 목록이 부분일치로 필터된다', async () => {
  ...
  await openPicker(root);
  const full = suggestedModels(root).length;
