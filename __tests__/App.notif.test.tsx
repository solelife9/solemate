...
describe('App 이 forecast/weekly 입력을 조립해 다른 종류의 표시 경로도 트리거한다', () => {
  test('shoe_replacement — 수명을 초과(overdue)한 신발이 있으면 포그라운드 진입 시 교체 Alert 를 띄운다', async () => {
    ...
    const shoes = [{id: 's1', name: 'Nike Pegasus', max_km: 100, start_km: 0}];
    const runs = [{id: 'r1', shoe_id: 's1', km: 500, run_date: localToday(), duration: 1800}];
    mockBackend(shoes, runs);
    // Firestore 정본 부팅: shoe_replacement forecast 는 부팅 캐시의 신발/런에서 조립된다
    // (REST 가 아님). 캐시에 시드해야 overdue 신발이 화면 상태로 올라온다(App.tsx:597-608).
