...
// 목표 링 진행 호의 '원시' strokeDashoffset. progress>1을 클램프하지 않으면 이 값이
// 음수가 되어 호가 한 바퀴를 넘겨 그려진다(over-fill) — 음수 여부로 클램프를 잡는다.
function goalRingOffset(root: ReactTestRenderer.ReactTestInstance): number {
  const arcs = root.findAll(
    (n: any) =>
      n.type &&
      n.type.displayName === 'Circle' &&
  ...
  if (!arcs.length) throw new Error('goal ring progress arc not found');
  return Number(arcs[0].props.strokeDashoffset);
}

// 목표 링 진행 호의 그라디언트 정지색(상태색) 배열. 진행 호의 stroke=url(#id)에서
// id를 떼어 같은 id의 LinearGradient를 찾고 Stop 색을 모은다. 달성(>=100%) 시
// 색이 GOOD로 바뀌므로 '채움'이 아니라 '상태 전환'을 검증할 수 있다.
function goalRingStops(root: ReactTestRenderer.ReactTestInstance): string[] {
  const arcs = root.findAll(
    (n: any) =>
      n.type &&
      n.type.displayName === 'Circle' &&
  ...
  if (!arcs.length) throw new Error('goal ring progress arc not found');
  const m = /#([^)]+)\)/.exec(String(arcs[0].props.stroke));
  const id = m ? m[1] : null;
  const grads = root.findAll(
    (n: any) => n.type && n.type.displayName === 'LinearGradient' && n.props.id === id,
  );
  if (!grads.length) throw new Error('goal ring gradient not found');
  return grads[0]
    .findAll((n: any) => n.type && n.type.displayName === 'Stop')
    .map((st: any) => String(st.props.stopColor));
}
...
test('주간 윈도우 밖 런은 달성률에서 제외된다(전체합이 아님)', async () => {
  ...
  expect(goalRingProgress(root)).toBeCloseTo(0.5, 2); // 링 진행도도 주간분만
});
...
test('목표 초과(>100%)는 실제 %를 보이되 링은 over-fill하지 않는다', async () => {
  ...
  // 링 진행도는 1.0에서 클램프 — >1로 과채움되지 않는다.
  expect(goalRingProgress(root)).toBeCloseTo(1, 2);
  // 원시 strokeDashoffset이 음수면 호가 한 바퀴를 넘겨 그려진다(over-fill). 0 이상이어야 한다.
  expect(goalRingOffset(root)).toBeGreaterThanOrEqual(0);
});
...
test('정확히 100%면 링이 GOOD(달성) 상태색으로 전환된다', async () => {
  ...
  const stops = goalRingStops(root);
  expect(stops).toContain(GOOD); // 달성 시 GOOD(녹색) 상태색으로 전환
  expect(stops).not.toContain(ACCENT); // 미달성 강조색(ACCENT)이 아님 — 단순 채움이 아닌 상태 전환
});
