...
// Stage 2b(Firestore 정본): addRun 은 더 이상 REST POST + 미동기 큐(loadPendingRuns)에
// 쓰지 않는다. 완주 런은 부팅 캐시(cache_runs_v1)에 즉시 durable 기록되고(App.tsx:750-761,
// "audit#3 미동기 큐의 역할을 대체"), cloudSync 가 Firestore 로 push 한다. 따라서 런의
// durable 영속·updatedAt 은 큐가 아니라 이 캐시에서 관찰한다.
async function loadCachedRuns(): Promise<any[]> {
...
test('addRun 은 완주 런에 updatedAt(epoch ms)을 찍어 부팅 캐시에 durable 기록한다', async () => {
  // Stage 2b: REST POST/미동기 큐가 제거됐다. 네트워크와 무관하게 완주 런은 부팅 캐시에
  // 즉시 durable 기록된다(크래시-세이프티). 백엔드는 빈 OK 로 둔다(부팅은 로컬-퍼스트).
  ...
  const cached = await loadCachedRuns();
  const saved = cached.find(r => r.km === 3.2);
  expect(saved).toBeTruthy();
  expect(saved.km).toBe(3.2);
  expect(typeof saved.updatedAt).toBe('number');
  expect(saved.updatedAt as number).toBeGreaterThan(0);
...
test('addShoe 는 생성 신발 레코드에 updatedAt 을 찍는다(id/name/max_km/start_km 보존)', async () => {
  ...
  // 첫 신발 등록은 '첫 신발' 업적을 해금해 셀러브레이션 오버레이(CelebrationScreen)가
  // Home 위에 뜬다(App.tsx:1545-1546). 닫아야 Home(onTab)으로 돌아간다 — '확인'(onClose).
  const celeb = renderer.root.findAll(n => n.props && typeof n.props.onClose === 'function' && n.props.data);
  if (celeb.length) {
    await act(async () => {
      celeb[0].props.onClose();
    });
    await tick(3);
  }
