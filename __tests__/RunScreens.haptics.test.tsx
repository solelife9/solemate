...
describe('RunGoalScreen — 런 시작 햅틱과 onStart 핸들러', () => {
  test("'러닝 시작' CTA 가 tap 햅틱 + onStart(기본 = 거리 탭 '자유' 프리셋, 목표 없음) 를 부른다", () => {
    // 민우님 2026-07-24 재확정: 거리·시간·스피드·트랙 4탭 + 거리 기본 선택 = '자유'(0km).
  ...
  test("거리 탭에서 5km 프리셋 선택 후 시작하면 onStart(5km)", () => {
    ...
    const root = render(<RunGoalScreen onStart={onStart} />).root;
    act(() => {
      pressableByLabel(root, '5km 목표 선택').props.onPress();
    });
    act(() => {
      pressableByLabel(root, '러닝 시작').props.onPress();
    });
  ...
  test('세그먼트/프리셋 버튼이 role=button + selected 상태를 노출한다', () => {
    ...
    const seg = pressableByLabel(root, '거리 목표'); // 기본 거리 모드 = 선택됨
    expect(seg.props.accessibilityRole).toBe('button');
    expect(seg.props.accessibilityState.selected).toBe(true);
    const free = pressableByLabel(root, '자유 러닝 선택'); // 거리 기본 = '자유' 칩 선택됨
    expect(free.props.accessibilityState.selected).toBe(true);
    act(() => {
      pressableByLabel(root, '5km 목표 선택').props.onPress();
    });
    expect(pressableByLabel(root, '5km 목표 선택').props.accessibilityState.selected).toBe(true);
  });
});
