...
test('상세 내구도 게이지는 남은 수명·컨디션을 렌더하고, 실효 마모 용어는 미노출', () => {
  ...
  expect(txt).toContain('최상의 컨디션'); // 양호 컨디션(상단 상태 칩)
...
// ── 9) 수명 직접 입력: 연필로 편집기를 펼친 뒤 숫자를 직접 입력하면 클램프 후 영속 ──
test('수명 편집기에서 숫자를 직접 입력하면 입력값(클램프)으로 onSetMaxKm을 호출한다', () => {
  const onSetMaxKm = jest.fn();
  const root = render(<ShoesScreen shoes={SHOES} runs={RUNS} onSetMaxKm={onSetMaxKm} />).root;
  tap(pressBy(root, 'Clifton 9')); // 상세 진입(미보관)

  // '남은 수명' 옆 연필(create-outline)로 수명 편집기 펼치기 — 펼치기 전엔 입력칸이 없다.
  expect(root.findAll((n: any) => n && n.props && n.props.accessibilityLabel === '신발 수명 직접 입력').length).toBe(0);
  tap(pressBy(root, 'create-outline'));

  // 직접 입력 250(km 단위 기본) → 범위(100~2000) 안이라 그대로 250으로 영속.
  const input = root.findAll((n: any) => n && n.props && n.props.accessibilityLabel === '신발 수명 직접 입력')[0];
  act(() => { input.props.onChangeText('250'); });
  act(() => { input.props.onBlur(); });

  expect(onSetMaxKm).toHaveBeenCalled();
  const calls = onSetMaxKm.mock.calls;
  expect(calls[calls.length - 1]).toEqual(['s2', 250]);
});

// 하한(100km) 아래 입력은 클램프된다.
test('수명 직접 입력이 하한(100km) 아래면 100으로 클램프된다', () => {
  const onSetMaxKm = jest.fn();
  const root = render(<ShoesScreen shoes={SHOES} runs={RUNS} onSetMaxKm={onSetMaxKm} />).root;
  tap(pressBy(root, 'Clifton 9'));
  tap(pressBy(root, 'create-outline'));
  ...
  const input = root.findAll((n: any) => n && n.props && n.props.accessibilityLabel === '신발 수명 직접 입력')[0];
  act(() => { input.props.onChangeText('30'); });
  act(() => { input.props.onBlur(); });

  const calls = onSetMaxKm.mock.calls;
  expect(calls[calls.length - 1]).toEqual(['s2', 100]);
});
