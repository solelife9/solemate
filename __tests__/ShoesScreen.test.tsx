...
test('락커 카드를 누르면 그 신발의 상세(모델 + 내구도 링)가 열린다', () => {
  ...
  // 게이지 설명문('쿠셔닝...')은 상세 텍스트 다이어트로 삭제(2026-07-04) — 부재를 고정.
  expect(txt).not.toContain('쿠셔닝');
});
...
// ── 4) retired shoe: 락커에서 제외(보관함으로 이관) → 시작 동선 분리, 기록 보존 ──
test('보관된 신발은 락커 목록·상세에서 제외된다(마이탭 보관함으로 이관)', () => {
  ...
  expect(() => pressBy(root, 'Nimbus 26')).toThrow(); // 락커 목록에 없음 → 상세 진입 불가
...
// ── 7) regression: 보관된 고마모 신발은 락커에서 빠져 keep-going 배너가 뜰 자리가 없다 ─
test('보관된 교체 신발은 락커에서 제외 → keep-going 교체 배너 미노출(보관됨과 모순 방지)', () => {
  ...
  expect(() => pressBy(root, 'Bondi 8')).toThrow(); // 락커에 없음
  // keep-going 교체 배너는 보관된 신발엔 어디에도 뜨지 않는다.
  expect(textOf(root)).not.toContain('지금 교체하면 부상 없이 계속 달릴 수 있어요');
});
