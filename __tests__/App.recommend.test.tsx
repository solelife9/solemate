...
test('ShoeDetail: 구매가 입력 → km당 비용 파생 표시 + 마지막 착용일 표시', async () => {
  ...
  // 구매가 입력 → costPerKm(150000, 5) = 30,000원/km 파생 표시
  const input = root.findAll(
    (n: any) => n && n.props && n.props.placeholder === '0' && typeof n.props.onChangeText === 'function',
  )[0];
  await act(async () => {
    input.props.onChangeText('150000');
  });
  await flush();
  const txt = textOf(root);
  expect(txt).toContain('30,000');
  expect(txt).toContain('원/km');
});
