...
test('신발 화면(전 화면 환산)도 토글 시 환산된 수치를 보여준다', async () => {
  ...
  // 탭 전환은 아이콘(footsteps)으로 — '신발' 라벨은 다른 화면 텍스트와 모호.
...
test('신발 상세 cost-per-km 힌트의 사용거리도 환산된다(km 하드코딩 버그 회귀 방지)', async () => {
  // mi 단위 + 구매가 영속 후 상세 진입. 사용거리 480km는 mi에선 298mi로 떠야 하고,
  // 비용 비율(1km당 N원)은 의도적으로 km 기준 유지. (이전 버그: 힌트가 '480km 사용'
  // 하드코딩 → hero/누적거리는 mi인데 이 힌트만 km = 한 화면 두 단위.)
  await AsyncStorage.setItem('settings_unit', 'mi');
  await AsyncStorage.setItem('price_s1', '120000'); // 120000원 / 480km = 250원/km
  const shoes: ApiShoe[] = [{id: 's1', name: 'Nike Pegasus', max_km: 600, start_km: 0}];
  const runs: ApiRun[] = [{id: 'r1', shoe_id: 's1', km: 480, run_date: '2026-05-20', duration: 7200}];
  const {root} = await mount(shoes, runs);

  // 신발 탭(footsteps) → 신발 카드(Pegasus) → 상세
  await tap(pressBy(root, 'footsteps'));
  await tap(pressBy(root, 'Pegasus'));
  const txt = textOf(root);

  // 사용거리는 환산: displayNum(480,'mi')=298 → '298mi 사용'
  expect(txt).toContain('298mi 사용');
  // 비용 비율은 km 기준 유지(환산 금지): 1km당 250원
  expect(txt).toContain('1km당 250원');
  // 버그 회귀 방지: km 하드코딩('480km 사용')이 다시 새지 않는다
  expect(txt).not.toContain('480km 사용');
});

test('계정 설정 행 — 기기/가입/버전 필드를 렌더한다', async () => {
  const {root} = await mount(SHOES, RUNS);
  await tap(pressBy(root, '프로필'));

  // 계정 설정 행을 펼친다
  await tap(pressBy(root, '계정 설정'));
  const txt = textOf(root);
  // 읽기 전용이라도 라벨/값 필드가 렌더돼야 한다(최소 기기 ID + 버전)
