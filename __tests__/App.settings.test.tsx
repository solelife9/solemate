...
async function mount(shoes: ApiShoe[], runs: ApiRun[]) {
  ...
  // Firestore 정본 부팅: 화면 데이터는 REST 가 아니라 부팅 캐시에서 읽힌다. 신발을 시드해야
  // 온보딩 게이트(!onboarded && shoes.length===0)도 건너뛴다(App.tsx:597-608, 1490).
...
test('단위 행 토글 → settings_unit 영속 + 전 화면 즉시 환산 반영', async () => {
  ...
  // (c) 다른 화면(홈)도 즉시 환산 단위로: 'mi 남았어요'(km 아님)
  // [카피 변경] 히어로 남은거리 문장이 'N km 남음' → '교체까지 약 N km 남았어요'로
  // 바뀌었다(HomeScreen.rn.tsx:194). 단위 환산 검증 의도는 그대로 유지한다.
  await tap(pressBy(root, '홈'));
  const home = textOf(root);
  expect(home).toContain('mi 남았어요');
  expect(home).not.toContain('km 남았어요');
...
test('홈은 토글 전 km 원숫자(595)를 보여준다(환산 기준점)', async () => {
  ...
  await tap(pressBy(root, '홈'));
  const home = textOf(root);
  // 기본 단위 km: 남은 수명 600-5 = 595km. 히어로 문장 '교체까지 약 595km 남았어요'
  // (HomeScreen.rn.tsx:194). 이전 'km 남음' 카피에서 변경됨.
  expect(home).toContain('595');
  expect(home).toContain('km 남았어요');
});
