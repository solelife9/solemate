...
export default function ProfileScreen({
  ...
  // 러닝화 찾기 진입. 원래는 스펙 비교가 신발 상세 밑바닥에, 후보 추천은 은퇴할 때만
      ...
      <ScrollView ref={scrollRef} contentContainerStyle={{ paddingTop: insets.top + 12, paddingHorizontal: GUTTER, paddingBottom: TABBAR_CLEARANCE, gap: rv(16) }}>
        ...
        {(onOpenProgression || onOpenHallOfShoes || onOpenMedalArchive || onOpenFindShoes || onOpenRanking) && (() => {
          const entries = [
            ...
            // 러닝화 찾기는 성취가 아니라 '도구'다. 그래도 같은 카드에 두는 이유:
            // 이 카드엔 제목이 없어 사용자에겐 '갈 곳 목록'으로 읽히고, 별도 카드 하나를
            // 더 세우면 표면만 늘어 여백을 먹는다(간결화 E1 과 같은 판단).
            onOpenFindShoes && {
              key: 'find-shoes', testID: 'open-find-shoes', icon: 'search-outline' as const,
              title: '러닝화 찾기',
              sub: '내 신발과 견줘서 다음 한 켤레',
              a11y: '러닝화 찾기 열기', onPress: onOpenFindShoes,
            },
