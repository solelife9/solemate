...
export default function RunEngine({shoe,insets,goalKm,goalMin=0,pacePlan=[],targetZone=0,track=null,indoor=false,weightKg,strideM=1,age=0,restHR=0,onSave,onDiscard,resume,resumeMode}:{shoe:{id:string;name:string};insets:any;goalKm:number;goalMin?:number;pacePlan?:number[];targetZone?:number;track?:{lapM:number}|null;indoor?:boolean;weightKg:number;strideM?:number;age?:number;restHR?:number;onSave:(km:number,dur:number,cad:number,memo:string,route:string,location:string,splits:{km:number;paceSec:number;elevM:number}[],elevM:number|null,cal:number,paceTrack:{d:number;t:number}[],hrTrack:{t:number;bpm:number}[],gapTrack:{d:number;t:number;e:number}[],trackMeta?:{lapM:number;laps:number;lapTimes:number[]}|null,cadTrack?:{t:number;spm:number}[])=>Promise<void>;onDiscard:()=>void;resume?:RunSnapshot|null;resumeMode?:'review'|'continue'}){
  ...
  /**
   * **이 화면 인스턴스가 엔진을 시작했는가.** beginRun 이 runTracker.start() 를 부른 뒤에만 true.
   *
   * 왜 필요한가(2026-08-04 QA 감사 Q-1, BLOCKER): runTracker 는 프로세스 싱글턴이고
   * (lib/runTracker.ts 하단), 누적 상태를 지우는 곳은 start() 하나뿐이다 — stop() 은
   * active 플래그만 내리고 거리·t0·경로를 그대로 둔다("저장을 위해 남긴다"는 계약).
   * 그런데 위치 권한이 거부되면 아래 마운트 효과가 beginRun 을 부르지 않고 빠져나간다.
   * 그 상태에서 종료를 누르면 finishRun 이 **직전 러닝이 남긴 값**을 읽어 새 기록으로
   * 저장했다 — 같은 러닝이 두 건이 되고 신발 거리가 두 번 차감된다(Iron Law 위반).
   * 화면엔 0.00km 가 떠 있어 사용자는 무엇이 저장됐는지도 알 수 없었다.
   *
   * 그래서 종료 경로는 "엔진이 내 것인가"를 먼저 묻는다. 싱글턴을 건드려 고치지 않는 이유는
   * stop() 이 데이터를 남기는 것 자체가 저장 흐름의 전제이기 때문이다 — 소유권을 아는 쪽은
   * 엔진이 아니라 화면이다.
   */
  ...
  // 스피드 코칭 상태(lib/paceCoach 순수 상태기). 런 시계(elapsed) 기준이라 일시정지 중엔
  // 시간이 흐르지 않아 자동으로 조용해진다. dt 는 직전 틱과의 차이로 계산한다.
  const paceCoachRef=useRef(initPaceCoachState());
  const prevPaceTickRef=useRef(0);
  ...
  useEffect(()=>{
    ...
    const unsub=runTracker.subscribe(ev=>{
      if(ev.type==='state'){
        ...
        setGpsStalled(s.stalled);setSnapshotFailing(s.snapshotFailing);
        // permLost 는 **두 곳에서** 온다: 주행 중 회수(엔진이 안다)와 시작 시 거부(화면만 안다).
        // 예전엔 엔진 값을 그대로 대입해서, 시작 거부 상태로 일시정지를 누르는 순간
        ...
        // 스피드 음성 코칭 — 판정은 lib/paceCoach(순수)가 한다(2026-07-27 분리).
        // 여기선 재생만: 목표 페이스 대비 '조금 빨라요/좋아요/조금 느려요'.
        // 스피드 탭에서만 동작한다(다른 목표 모드는 pacePlan 이 비어 있다).
        if(pacePlan&&pacePlan.length&&!s.paused){
          const dtPace=Math.max(0,s.elapsed-prevPaceTickRef.current); prevPaceTickRef.current=s.elapsed;
          const d=decidePaceCoach(
            paceCoachRef.current,
            currentTargetPace(pacePlan,s.dist),
            s.currentPaceSecPerKm,
            s.elapsed,
            dtPace||1,
          );
          paceCoachRef.current=d.state;
          if(d.announce==='slow')runVoice.paceSlow();
          else if(d.announce==='fast')runVoice.paceFast();
          else if(d.announce==='on')runVoice.paceOn();
        }
  ...
  async function finishRun(){
    // ── 엔진 소유권 확인이 가장 먼저다(Q-1) ──────────────────────────────────
    // 엔진을 시작한 적이 없으면(위치 권한 거부로 beginRun 미도달, 또는 beginRun 이 start()
    // 전에 실패) 아래 getDistanceKm()/getElapsedFinal() 은 **직전 러닝의 값**을 돌려준다.
    // 그걸 저장하면 같은 러닝이 두 건이 된다. 저장 경로를 아예 타지 않고 닫는다.
    if(!engineStartedRef.current){
      stop();
      if(resume){
        // '이어 달리기'가 시작에 실패한 경우 — **복구하려던 기록은 스냅샷에 그대로 있다.**
        // 여기서 버리면 크래시로 잃을 뻔한 러닝을 우리 손으로 지우는 셈이다. 이어 달리는 것만
        // 불가능한 것이지 기록이 없는 게 아니므로, 검토 화면으로 넘겨 저장/버리기를 사용자가
        // 고르게 한다(fin* 는 이미 스냅샷 값으로 초기화돼 있다).
        if(trackMode&&resume.track){
          const tk=resume.track;
          setFinKm((tk.lapTimes.length*tk.lapM)/1000);
          setFinPaceTrack(lapsToTrack(tk.lapTimes,tk.lapM/1000));
        }
        setPhase('done');
        showDialog(
          '이어 달릴 수 없어요',
          permLost
            ? '위치 권한이 없어 이어 달리기를 시작하지 못했어요. 여기까지의 기록은 저장할 수 있어요.'
            : '러닝을 다시 시작하지 못했어요. 여기까지의 기록은 저장할 수 있어요.',
        );
        return;
      }
      showDialog(
        '기록된 러닝이 없어요',
        permLost
          ? '위치 권한이 없어 이번 러닝은 기록되지 않았어요. 설정에서 위치를 허용하면 다음부터 기록할 수 있어요.'
