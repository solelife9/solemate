 ...
 *    않았다.
 *  2026-07-28 0.8 → 0.5: **실측 러닝에서 과대가 아니라 과소로 넘어가 있었다.**
 ...
 *    0.5 로 낮추면 하한이 최대 10m(≈3~4 fix)로 줄어 코너 손실이 절반쯤 회복된다.
 *    0.35(구값)까지 내리지 않은 이유: 그 값에서 +9% 과대가 재발했기 때문이다. 0.5 는
 *    과대(0.35)와 과소(0.8) 사이를 노린 값이고, **다음 실측 러닝으로 검증해야 한다** —
 *    합성 스윕만 믿고 정한 값이 두 번 연속 빗나갔으므로 실측이 유일한 판정 기준이다. */
