...
/**
 * 폰 런과 워치 런이 **같은 러닝일 때 하나로 합칠지**(`lib/runMerge`).
 *
 * **정상값은 `true` 다.** 끄면 같은 러닝이 두 건으로 저장되고 **신발 거리가 두 번 차감**된다
 * (2026-07-28 실측: 5.4km 러닝이 신발에 10.50km). 그걸 막으려고 만든 장치다.
 *
 * ── `false` 로 두는 유일한 경우: **거리 정확도 실측** ────────────────────────────
 * 병합은 거리·시간을 **워치 값으로 덮는다**(워치가 더 정확해서 — 실측 폰 −5.3% vs 워치
 * −1.3%). 그래서 워치를 차고 뛰면 **폰이 잰 값이 저장된 기록에 남지 않는다.**
 * `PHANTOM_ACC_FLOOR_FACTOR`(현재 0.6, 미검증)를 실측으로 판정하려면 폰 값이 필요하므로,
 * 측정 러닝 동안만 끈다. 두 건이 각각 경로·스플릿·페이스 곡선까지 온전해 비교가 쉽다.
 *
 * **측정이 끝나면 반드시 `true` 로 되돌리고, 중복 런 한 건을 지운다.**
 * 신발 누적 거리는 런에서 파생되므로(`lib/shoe.shoeHealth`) 지우면 자동으로 제자리다 —
 * `deleteRun` 이 최고수위(`usedKmHighWater`)도 함께 내려 '설명되지 않는 감소' 경고도 안 뜬다.
 *
