...
export const liveActivity = {
  ...
  start(shoeName: string, goalKm: number, distanceKm: number, elapsedSec: number, paceLabel: string, avgPaceLabel: string, cadenceSpm: number = 0, bpm: number = 0) {
    if (!available) return;
    try { M.start(shoeName || '', goalKm || 0, distanceKm || 0, elapsedSec || 0, paceLabel || '--', avgPaceLabel || '--', cadenceSpm || 0, bpm || 0); } catch { /* noop */ }
  },
  /** 진행 상태 갱신(거리/시간/페이스/케이던스/심박 0=미측정). 호출자가 throttle 한다. */
  update(distanceKm: number, elapsedSec: number, paceLabel: string, avgPaceLabel: string, cadenceSpm: number = 0, bpm: number = 0) {
    if (!available) return;
    try { M.update(distanceKm || 0, elapsedSec || 0, paceLabel || '--', avgPaceLabel || '--', cadenceSpm || 0, bpm || 0); } catch { /* noop */ }
  },
  /** 러닝 종료/완주/취소 — 위젯을 닫는다. */
  end() {
    if (!available) return;
