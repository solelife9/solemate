...
export interface RunSnapshot {
   ...
   *
   * 왜 필요한가: 완주 시 평균 케이던스를 '총 걸음수 ÷ 이동 시간'으로 내는데, 분자는
   * OS 만보계의 러닝 전체 누적(일시정지 중 걸은 것 포함)이고 분모는 일시정지를 뺀
   * 이동 시간이었다. 30분@170spm 뒤 10분 걸어서 물 마시면 **203 spm** 이 저장된다.
   * 그리고 크래시 복구 런은 걸음 기준점만 리셋되고 분모는 런 전체라 반대로 **24 spm**
   * 이 저장됐다. 분자와 분모가 서로 다른 구간을 덮고 있었던 것이다.
   *
   * 이 값을 스냅샷에 실어야 '이어 달리기'가 걸음 누적을 잇는다(없으면 복구 런의
   * 케이던스가 다시 무너진다). 구 스냅샷엔 없다 — 없으면 0에서 시작(예전과 같음).
   */
  movingSteps?: number;
