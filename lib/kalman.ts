...
// 측정 노이즈를 좇아 출렁이지 않을 만큼은 작게.
// 2026-07-11 재튜닝: 실측 비교런(NRC 동시 주행)에서 +9% 과대가 확정돼 합성 GPS 하네스
// (__tests__/helpers/gpsSynthetic — 시드 RNG·시간상관 노이즈·9개 시나리오)로 1.4→0.7 로
// 낮췄다. 0.7 은 코너/인터벌 추종을 유지하면서(원형 곡선 오차 ±0.4%) 지터 적산을 줄이는
// 스윕 최적점 — 이보다 낮추면 곡선에서 NIS 게이트 오작동으로 오히려 과대/과소가 커진다.
// 거리 정확도의 나머지 반쪽은 거리 적산단의 5점 중심 평활(lib/distanceSmoother)이 담당.
export const ACCEL_PSD = 0.7 * 0.7;
