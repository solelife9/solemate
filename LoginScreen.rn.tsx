  ...
  return (
    <View style={[st.screen, {paddingTop: insets.top + 24, paddingBottom: insets.bottom + 24}]} testID="login-screen">
      ...
      <View style={st.actions}>
        ...
        {/* 만 14세 확인은 **계정이 만들어지기 전**에 있어야 한다(2026-08-07).
            개인정보보호법 §22-2 는 만 14세 미만의 개인정보를 법정대리인 동의 없이 처리하지
            못하게 하고, 이 화면의 버튼을 누르는 순간 Firebase 계정이 만들어지며 식별자·
            이메일 수집이 시작된다. 온보딩(로그인 **뒤** 화면)에 같은 문구가 있었으나
            그때는 이미 수집이 끝난 뒤라 순서가 뒤집혀 있었다 — M-5(약관 링크)와 같은
            종류의 실수이고, 그 수정에서 이 한 줄만 빠졌다.
            온보딩의 문구는 위치기반서비스 약관 동의를 겸하므로 그대로 둔다(중복이 아니라
            다른 동의다). */}
        <Text style={st.legal}>
          만 14세 이상이며, 계속하면{' '}
          <Text
