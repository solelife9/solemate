...
const CAPS = {
  ...
  shoeHealth: 100,
  collection: 500,
  // 실제 획득 가능한 최대 XP 는 **≈6,310** 이다(lib/progression/rank.ts — 마라톤 +
  // 10켤레 은퇴·동행 + 모든 experience, 약 3.75년치). 상한이 10,000 이었던 것은 근거
  // 없는 여유였고, 그 여유가 곧 **어떤 진짜 사용자도 평생 못 미치는 점수를 쓸 수 있는
  // 폭**이었다(2026-08-08).
  //
  // 7,000 으로 조인다 — 6,310 에 딱 맞추지 않는 이유는 **조이는 것이 clamp 이지 거부가
  // 아니기 때문**이다. 상한을 실제 최대에 붙였다가 그 추정이 조금이라도 낮으면, 정직하게
  // 달성한 사람의 점수를 조용히 깎아 내려 표시한다(Truth only 위반). 여유 ~11% 는
  // 업적이 추가돼 최대치가 올라가도 버틸 만큼이고, 조작 폭은 10,000 대비 30% 줄어든다.
  //
  // ⚠️ 이건 구멍을 **좁히는** 것이지 막는 게 아니다. 이 축만 서버가 검산하지 못한다
  // (근거는 위 헤더 · 감사 L-7b). 막으려면 축을 내리거나 엔진을 서버로 옮겨야 한다.
  progressPoints: 7000,
};
