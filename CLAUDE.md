...
- **정체성(2026-07-09 확정):** 다크(`BG #0A0A0A`, 카드 `#1C1C1E`/`#2C2C2E`) + **무채 글래스 베이스**, `ACCENT #FFFFFF`(흰=강조, 모노크롬 시스템). 프리미엄은 재질·타이포·절제(Apple급). 브랜드색은 **러닝 링 전용 Keego Ember `RING_ACCENT #FF8000`**(링 그라데이션 `#FFB458→#E56600`, 구 'McLaren 파파야' 호칭 폐기 — BRAND.md §4). 바이올렛 전역 액센트(`#8B5CF6`)는 "남발돼 튄다" 실기기 피드백으로 당일 폐지.
