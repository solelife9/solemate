...
- **정체성(2026-07-09 갱신):** 다크(`BG #0A0A0A`, 카드 `#1C1C1E`/`#2C2C2E`) + **무채 글래스 베이스**. 프리미엄은 재질·타이포·절제(Apple급). 브랜드 액센트 = **일렉트릭 바이올렛 `#8B5CF6`**(오렌지 폐지 — Strava 인접·무채 방향과 어긋남).
- **액센트 절제:** 바이올렛은 러닝 링 등 시그니처 순간에만. 색은 의미에만(조건색). 라벨·보조는 T3 회색(`#9C9CA3`). HoF Legend는 골드/오렌지 유지(성취 도메인).
