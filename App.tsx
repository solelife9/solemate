...
import {setAnalyticsEnabled,trackFirstShoeAdded,trackShoeRetired,trackCrashRecovery,trackSyncFailed} from './lib/productAnalytics';
import {
...
import {hkSaveRunWorkout, hkBackfillHeartRate, hkFindRunWorkoutWindow} from './lib/health';
...
function Main(){
  ...
  // 심박 지연 보정 도구(2026-08-09 분해) — 워치 수신·HK 백필·폰 런 저장·상세 스윕까지
  // 일곱 군데가 쓴다. **쓰는 곳보다 먼저** 둔다(예전엔 파일 한복판에 있었다).
  // useEffect 가 없는 순수 도구라 이 위치가 부수효과 순서를 바꾸지 않는다.
  const {runsForHrRef,hkBackfillAndRepair}=useHeartRateRepair(runs,setRuns);
  ...
  /**
   * 신발 표시 이름 변경 — **`name` 이 아니라 `nickname` 을 고친다**(2026-08-09).
   *
   * 예전엔 `name` 을 통째로 덮어썼다. 그런데 `name` 은 카탈로그 정체성이고, 거기서
   * 파싱한 `brand|model` 이 공식 스펙(무게·힐스택·드롭)과 카테고리를 조회하는 키다.
   * 같은 모델 두 켤레를 구별하려고 이름을 바꾸는 순간 — 정확히 그러라고 만든 기능이다 —
   * 스펙 조회가 **조용히 빗나가** 다음 신발 비교에서 축이 사라졌다. 화면엔 아무 표시도
   * 없어서 알아챌 수 없는 종류다.
   *
   * 빈 문자열이면 별명을 **지운다**(기본 표시로 돌아간다) — 지우는 방법이 없으면
   * 한 번 붙인 별명에 갇힌다.
   */
  async function updateShoeName(id:string,nickname:string){
    const v=nickname.trim();
    setShoes(prev=>prev.map(s=>{
      if(s.id!==id) return s;
      const next={...s} as BackendShoe;
      if(v) next.nickname=v; else delete next.nickname;
  ...
  // ── 폰 바깥 표면(워치·홈 위젯) 연동 ─────────────────────────────────────────
  // 신발 목록·최근 러닝·위젯 푸시 + 워치 단독 러닝 수신은 useWatchSync 가 진다
  // (2026-08-09 분해). 신발 이중 차감 방어 두 겹(runId 중복 + 시간창 병합)도 그 안에 있다.
  useWatchSync({homeShoes,homeActiveIdx,age,restHR,effectiveId,runs,shoes,
  ...
  useEffect(()=>{
    ...
    // runsForHrRef 는 ref(정체성 고정) — dep 에 넣어도 스윕이 다시 예약되지 않는다.
  },[authUser?.uid,runsForHrRef]);
  ...
  // ── 러닝 시작 관문(권한 확인 → 카운트다운) ──────────────────────────────────
  // '시작'과 실제 러닝 사이의 분기 여섯(실내/야외 × 권한 미결정/거부/허용)은
  // useRunEntryGate 가 진다(2026-08-09 분해). 야외는 카운트다운 **전에** 묻고,
  // 실내는 위치가 아니라 걸음을 본다 — 둘 다 실기기에서 다친 뒤 생긴 규칙이다.
  const {startActiveRun,requestLocationThenStart}=useRunEntryGate({
