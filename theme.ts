...
// ── 아이콘 크기 스케일 ────────────────────────────────────────────────────────
// 색·폰트·간격은 토큰인데 아이콘 크기만 화면 리터럴로 24가지 값(11~46)이 흩어져 있었다
// (2026-07-26 정리). 특히 15·16·17·18 네 값이 98곳을 차지했는데 눈으로는 구분이 안 되는
// 1pt 차이였고, 새 아이콘을 넣을 때마다 옆 파일을 베껴 값이 하나씩 늘었다.
// 여섯 단계로 수렴한다 — 크기가 아니라 '쓰임'으로 이름 붙여 고를 때 헷갈리지 않게.
// 소비처는 size={ri(ICON.x)} 로 쓴다(반응형 스케일은 종전대로 ri 가 담당).
// 마이그레이션 매핑: 11·12·13·14 → tag / 15·16·17 → inline / 18·19·20 → action /
// 22·24 → nav / 26·28 → feature / 36·38·40·46 → hero. 대부분 ±1pt(시각 동등).
// 제외한 6곳(스윕 후 남은 리터럴 전부): 업적 글리프 62 · 메달 원반 168 · 링 272 는 아이콘이
// 아니라 도형이고, 아바타 자리표시 30 · 재개 버튼 play 32 · 권한 안내 44 는 각각 담긴 원형
// 버튼/화면 크기에 묶인 1회성 치수다. 셋 다 **한 번씩만 등장**해 값이 번질 여지가 없으므로
// 억지로 램프에 끼우지 않는다(끼우면 −4~−10pt 시각 변화만 생긴다).
export const ICON = {
  /** 13 — 칩·태그·캡션 옆의 아주 작은 표식. */
  tag: 13,
  /** 16 — 본문·라벨 인라인, 리스트 행 보조 아이콘(최빈). */
  inline: 16,
  /** 19 — 헤더 아이콘 버튼·행 액션(탭 가능한 것들). */
  action: 19,
  /** 23 — 탭바·네비게이션 급. */
  nav: 23,
  /** 26 — 카드 피처·빈 상태 일러스트 대용. */
  feature: 26,
  /** 40 — 대형 빈 상태·성취 표식. */
  hero: 40,
} as const;
...
/**
 * 최소 터치 타깃(pt). Apple HIG 44pt · Android 48dp 의 접근성 **하한**이다.
 *
 * ⚠️ **rs()/rms() 로 감싸지 마라**(2026-08-07 감사). 그건 화면 폭 비례 스케일이라
 * 작은 폰에서 44 → 40pt 로 **줄어든다.** 이 값은 디자인 치수가 아니라 "손가락이 닿는
 * 최소 크기"이고, 화면이 작다고 손가락이 작아지지 않는다. 오히려 작은 화면에서
 * 오터치가 더 잘 난다 — 줄이면 정확히 반대 방향이다.
 * 승격해 놓고 rs() 로 다시 무효화한 이력이 있어 회귀 가드를 뒀다:
 * __tests__/touchTargetFloor.test.ts
 */
