...
struct RunView: View {
  ...
  private var mainPage: some View {
    // ── AOD 갱신에 **하한을 준다** (2026-08-08 · 서술 정정 2026-08-09) ──────────
    // 예전엔 TimelineView 가 경과시간 한 줄만 감쌌다.
    //
    // ⚠️ **정정**: 처음엔 "그래서 AOD 에서 거리·페이스가 얼어붙었다"고 적었는데 그건
    // **확인되지 않은 주장**이다(감사 서술을 실기기 검증 없이 옮겼다). 거리·페이스는
    // WorkoutManager 의 @Published 값이라 데이터가 도착하면 SwiftUI 가 다시 그리고,
    // AOD 에서도 HK 워크아웃 빌더·위치 콜백은 계속 온다. 즉 전에도 갱신됐을 가능성이 크다.
    //
    // 이 변경이 실제로 하는 일은 **하한 보장**이다: 콜백이 throttle 되는 구간에도
    // 최소 60초마다 다시 그린다. TimelineView 는 다른 갱신을 막지 않으므로
    // (데이터가 오면 그때도 그린다) **더 나빠질 수는 없고**, 끊기는 구간에서만 좋아진다.
    // AOD 는 어차피 60초 간격이라(watchOS 가 그보다 잦은 갱신을 허용하지 않는다)
    // 전력 비용도 사실상 그대로다.
