...
- 로컬 저장: AsyncStorage(로컬-퍼스트 캐시). 백엔드 정본: **Firestore**(Phase 5b 이후 — 신발/런·
  랭킹·동기 모두 Firestore). Render REST(solelife-backend)는 Stage 0 일회성 이관용으로만 잔존.
...
- `AUTO_PAUSE_SPEED_MPS = 0.6`, `AUTO_PAUSE_HOLD_S = 3`, `AUTO_RESUME_SPEED_MPS = 1.0`, `AUTO_RESUME_HOLD_S = 1`
...
- ~~**단일 진실원천 = REST 백엔드.**~~ **[Phase 5b 로 폐기]** 신발/런 정본은 이제 **Firestore**다.
  쓰기·부팅·새로고침 모두 로컬-퍼스트 + cloudSync(Firestore push). REST 역등록(`apiAddShoe`/
  `apiAddRun`)·`backRegisterMerged`·`syncPendingRuns` 는 제거됨. (이 항목은 audit-hardening 당시
  규약으로, 현재는 Firestore 정본이 우선한다.)
