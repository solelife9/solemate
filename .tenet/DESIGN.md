...
- **콘텐츠 리스트 화면은 상단정렬이 기본.** 세로 공백을 `justifyContent:'center'`(통째 센터링)로 숨기지 말 것 — 콘텐츠가 화면 중앙에 붕 떠 보인다. 공백은 **타이포·간격 상향 또는 요약/내용 추가**로 채운다(예: 메달 아카이브·신발 보관함 상단 요약 스탯 헤더). **0개 빈 상태만 중앙 히어로 허용**(관용).
