...
export default function RunCountdownScreen({
  ...
  const gpsCol = phase !== 'gps' || gps >= 3 ? C.sage : C.t3;
  ...
  return (
    <View style={s.screen}>
      ...
      {/* top */}
      <View style={s.top}>
        <Pressable onPress={cancel} hitSlop={8} style={s.cancel}>
          <Icon name="back" size={18} color={C.t2} /><Text style={s.cancelText}>취소</Text>
        </Pressable>
        <View style={s.shoeChip}><ShoeGlyph color={C.t2} /><Text style={s.shoeText}>{shoeLabel}</Text></View>
      </View>
      ...
      <View style={s.center}>
        <View style={s.dial}>
          <Svg width={DIAL} height={DIAL} style={{ transform: [{ rotate: '-90deg' }] }}>
            <Circle cx={DIAL / 2} cy={DIAL / 2} r={R} stroke="rgba(255,255,255,0.07)" strokeWidth={STROKE} fill="none" />
            <AnimatedCircle cx={DIAL / 2} cy={DIAL / 2} r={R} stroke={C.accent} strokeWidth={STROKE} fill="none"
              strokeLinecap="round" strokeDasharray={DASH} strokeDashoffset={dialOffset} />
          </Svg>
          <View style={s.dialFace}>
            {phase === 'gps' ? (
              <View style={s.pin}><Icon name="target" size={30} color={C.accent} /></View>
            ) : phase === 'go' ? (
              <Animated.Text style={[s.go, { transform: [{ scale: goScale }] }]}>GO</Animated.Text>
            ) : (
              <>
                <Animated.Text style={[s.count, { opacity: numOpacity, transform: [{ scale: numScale }] }]}>{num}</Animated.Text>
        ...
        <View style={s.gpsRow}>
          <View style={s.bars}>
            {[8, 12, 16, 20].map((h, i) => (
              <View key={i} style={{ width: 4, height: h, borderRadius: 2, backgroundColor: (phase !== 'gps' || i < gps) ? C.sage : 'rgba(255,255,255,0.16)' }} />
            ))}
          </View>
        ...
        {/* goal chips */}
        <View style={s.chips}>
          <View style={s.chip}><Icon name="target" size={14} color={C.t3} /><Text style={s.chipText}>목표 <Text style={s.chipB}>{goalKm}.0 km</Text></Text></View>
          <View style={s.chip}><Icon name="route" size={14} color={C.t3} /><Text style={s.chipText}>{outdoor ? '야외 러닝' : '실내 러닝'}</Text></View>
        </View>
      </View>
      ...
      <View style={s.foot}>
        {phase === 'gps' && (
          <>
            <Pressable onPress={skip} style={({ pressed }) => [s.startNow, pressed && { opacity: 0.92 }]}>
              <Icon name="play" size={22} color="#fff" /><Text style={s.startNowText}>지금 시작</Text>
            </Pressable>
            <Pressable onPress={skip} hitSlop={8}><Text style={s.skip}>카운트다운 건너뛰기</Text></Pressable>
          </>
        )}
      </View>
    </View>
...
const s = StyleSheet.create({
  screen: { flex: 1, backgroundColor: C.bg, paddingTop: 60, paddingBottom: 34 },
  top: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 22 },
  cancel: { flexDirection: 'row', alignItems: 'center', gap: 6, height: 34, paddingLeft: 10, paddingRight: 14, borderRadius: 999, backgroundColor: 'rgba(255,255,255,0.05)', borderWidth: StyleSheet.hairlineWidth, borderColor: C.hair },
  cancelText: { color: C.t2, fontFamily: UI, fontSize: 13, fontWeight: '500' },
  shoeChip: { flexDirection: 'row', alignItems: 'center', gap: 7, height: 34, paddingHorizontal: 14, borderRadius: 999, backgroundColor: C.surface, borderWidth: StyleSheet.hairlineWidth, borderColor: C.hair },
  shoeText: { color: C.t2, fontFamily: DP, fontSize: 13, fontWeight: '600' },
  ...
  pin: { width: 62, height: 62, borderRadius: 999, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(255,101,0,0.12)', borderWidth: StyleSheet.hairlineWidth, borderColor: 'rgba(255,101,0,0.3)' },
  count: { color: C.t1, fontFamily: DP, fontSize: 150, fontWeight: '600', letterSpacing: -4, lineHeight: 156, includeFontPadding: false },
  countLabel: { color: C.t3, fontFamily: UI, fontSize: 13, fontWeight: '500', marginTop: 2 },
  go: { color: C.accent, fontFamily: DP, fontSize: 104, fontWeight: '700', letterSpacing: -1, includeFontPadding: false },

  gpsRow: { flexDirection: 'row', alignItems: 'center', gap: 9, marginTop: 30 },
  bars: { flexDirection: 'row', alignItems: 'flex-end', gap: 3, height: 20 },
  gpsText: { fontFamily: UI, fontSize: 13, fontWeight: '600' },

  chips: { flexDirection: 'row', gap: 8, marginTop: 14 },
  chip: { flexDirection: 'row', alignItems: 'center', gap: 7, height: 32, paddingHorizontal: 14, borderRadius: 999, backgroundColor: 'rgba(255,255,255,0.04)', borderWidth: StyleSheet.hairlineWidth, borderColor: C.hair },
  chipText: { color: C.t2, fontFamily: UI, fontSize: 12.5, fontWeight: '500' },
